...
class QueryBase(QueryMixin):
    ...
    def event_counts(self, id):
        ...
        SELECT
            ...
            SUM(employee_events.positive_events) AS sum_positive_events,
            SUM(employee_events.negative_events) AS sum_negative_events
        FROM {self.name}
        JOIN employee_events
            ON {self.name}.{self.name}_id = employee_events.{self.name}_id
        WHERE {self.name}.{self.name}_id = {id}
        GROUP BY employee_events.event_date
        ORDER BY employee_events.event_date
        """
